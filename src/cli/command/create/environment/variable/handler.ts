...
export async function createOrUpdateEnvironmentVariable(
  ...
  if (loadFile) {
    const filePath = await input({
      message:
        'Enter the path to the JSON file with the environment variable settings:'
    });

    const resolvedPath = path.resolve(filePath);

    variables = await Bun.file(resolvedPath).json();
  } else {
    const tempFolder = path.resolve('temp');
    const templateFolder = path.resolve('template');
    const templateName = 'environment-variable.json';
    ...
    variables = await Bun.file(tempFilePath).json();
  ...
  const selectedIds = await checkbox({
    message: 'Select in which repositories you want to create the variable',
    choices: choices,
    loop: false,
    pageSize: 20
  });
